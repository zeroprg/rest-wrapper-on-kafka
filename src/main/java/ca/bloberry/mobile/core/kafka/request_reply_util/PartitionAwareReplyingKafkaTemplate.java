package ca.bloberry.mobile.core.kafka.request_reply_util;
...
public class PartitionAwareReplyingKafkaTemplate<K, V, R> extends ReplyingKafkaTemplate<K, V, R> implements PartitionAwareReplyingKafkaOperations<K, V, R> {
  ...
  @Override
  public RequestReplyFuture<K, V, R> sendAndReceiveDefault( V data) {
    return sendAndReceive(getDefaultTopic(), data);
  }

  @Override
  public RequestReplyFuture<K, V, R> sendAndReceiveDefault(K key,  V data) {
